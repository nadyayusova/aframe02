AFRAME.registerComponent('auto-gyro-detect', {
  init() {
    const el = this.el;
    ...
    const hasGyroscopeSupport = () => {
      return new Promise((resolve) => {
        if (
          ...
          // Android — слушаем событие один раз
          const handler = (event) => {
            window.removeEventListener('deviceorientation', handler);
            resolve(event && event.alpha !== null);
          };
          window.addEventListener('deviceorientation', handler);
          // fallback, если ничего не пришло
